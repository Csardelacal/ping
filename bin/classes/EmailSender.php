...
class EmailSender
{
	...
	public function push($email, $src, $content, $url, $media = null) {
		...
		$this->sso->sendEmail($email, sprintf('[%s] %s %s', Environment::get('site.name')? : 'Ping', $src? $src->getDisplayName() : 'Someone', Strings::ellipsis($content, 120)), $t);
	}
